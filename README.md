...
5. Clone this repo and open a terminal in the `todo-cljs` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/todo_cljs/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns todo-cljs.example
  (:require [todo-cljs.config :as config])
